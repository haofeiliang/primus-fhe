...
impl<T: FheUint> HybridCrtGlweKeySwitchingKey<T> {
    ...
    fn validate_key_switch<M, Table>(
        ...
        hybrid_params: &HybridRNS<T, M>,
        table: &Table,
        context: &HybridCrtGlweKeySwitchingContext<T>,
    ) where
        M: FieldContext<T>,
        Table: DcrtTable<ValueT = T>,
    {
        ...
        let output_dimension = self.output_qp_rns_glwe_mid / qp_rns_poly_len;
        ...
        assert_eq!(context.output_dimension, output_dimension);
        ...
        Table: DcrtTable<ValueT = T>,
    {
        ...
        let qp_moduli = hybrid_params.qp_base().moduli();
        let HybridCrtGlweKeySwitchingContext {
            accumulator_qp,
        ...
        B: RawData<Elem = T> + DataMut,
    {
        ...
        c_out.set_zero();
        let output_rns_glwe_mid = output_dimension * q_rns_poly_len;
        let (accumulator_mask, accumulator_body) = context
            .accumulator_qp
            .split_at(output_dimension * qp_rns_poly_len);
        ...
        B: RawData<Elem = T> + DataMut,
    {
        ...
        let (mask_in, body_in) = c_in.a_b(self.input_rns_glwe_mid);
    ...
    pub fn key_switch_coeff_inplace<M, Table, A, B>(
        &self,
        c_in: &CrtGlweCiphertext<A>,
        c_out: &mut DcrtGlweCiphertext<B>,
        hybrid_params: &HybridRNS<T, M>,
