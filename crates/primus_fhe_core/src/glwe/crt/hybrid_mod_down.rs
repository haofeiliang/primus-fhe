    ...
    Table: DcrtTable<ValueT = T>,
{
    ...
    let p_len = hybrid_params.p_moduli_count() * poly_length;
    ...
    hybrid_params.approx_convert_p_to_q(polynomial_mod_p, converted_p_mod_q, poly_length, scratch);
...
mod tests {
    ...
    fn mixed_domain_mod_down_matches_coefficient_reference() {
        ...
        let hybrid_params = HybridRNS::new(&q_moduli, &p_moduli, 2).unwrap();
        ...
        let mut reference_scratch = vec![0; hybrid_params.mod_down_scratch_len(POLY_LENGTH)];
        ...
        let mut scratch = vec![0; hybrid_params.mod_down_scratch_len(POLY_LENGTH)];
        approx_mod_down_ntt(
            &hybrid_params,
            &table,
            &mut mixed_domain,
