...
生成的 `reduce_add_mul_slice_assign` 和 `reduce_mul_add_slice_to` 与 `primus_modulus` 共用 x86_64 原生内核，稳定版构建同样适用。对于至少 32 项的切片和非 2 的幂模数，`u32` 在支持时使用 AVX-512F；`u64` 仅在 `q < 2^50` 且 CPU 支持 AVX-512F/DQ/IFMA 时使用 IFMA。类型与常量模数资格检查会在优化构建中消除。生成的 context 仍为零大小，不增加预计算存储或 scratch。
