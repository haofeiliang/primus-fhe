...
GLWE-based TFHE with the native torus.
...
The [basic example](examples/fourier_basic.rs) shows parameters → context → paired keys →
...
Run the [17-threshold example](examples/fourier_mvb_thresholds.rs) with `--example fourier_mvb_thresholds`.
...
[CBS → CMUX example](examples/fourier_circuit_bootstrap.rs) shows their complete consumption chain.

The example uses classic keys in both orders and reuses the LWE input, GGSW control
and GLWE selection buffers. For sparse CBS, use the explicit key generation workflow
in [Experimental sparse PBS](#experimental-sparse-pbs), passing `Some(cbs_config)`
instead of `None`, then create `context.circuit_bootstrap_evaluator(&server)`.
The CBS and CMUX calls are the same.

The output is `FourierGgsw` under the accumulator secret. The key binds output layout
and trace/scheme-switch bases; advanced `try_from_parts` can use another output basis with the same level count.
