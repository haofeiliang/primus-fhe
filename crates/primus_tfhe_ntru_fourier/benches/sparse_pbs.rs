...
//!
//! cargo bench -p primus_tfhe_ntru_fourier --bench sparse_pbs
//! SIMD: cargo +nightly bench -p primus_tfhe_ntru_fourier --bench sparse_pbs --features simd
...
fn backend<T: TorusFftValue, Table: FftTable>(c: &mut Criterion, fft_name: &str) {
    const N: usize = 1024;
    ...
    for sparse in [false, true] {
        ...
        for (message, input) in inputs.iter().enumerate() {
            for count in [1, 3] {
                ...
                for (i, cipher) in results.iter().enumerate() {
                    ...
                    let error = phase.wrapping_sub(encoded).into_torus_f64().abs();
                    let index = usize::from(count == 3);
                    max_error[index] = max_error[index].max(error);
                }
            }
        }
        eprintln!(
        ...
        let mut keygen_rng = StdRng::seed_from_u64(0xB80301);
...
fn sparse_pbs(c: &mut Criterion) {
    ...
    backend::<u32, TfheFftTable>(c, "tfhefft");
    backend::<u64, RustFftTable>(c, "rustfft");
    backend::<u64, TfheFftTable>(c, "tfhefft");
}
