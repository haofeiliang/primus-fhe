...
[basic 示例](examples/ntt_basic.rs)展示参数 → context → 配对密钥 → 客户端 → 单个 LUT →
...
运行[17 阈值示例](examples/mvb_thresholds.rs)，命令使用 `--example mvb_thresholds`。
...
[CBS → CMUX 示例](examples/ntt_circuit_bootstrap.rs)展示完整消费链。

示例使用经典密钥。稀疏 CBS 沿用[实验性稀疏 PBS](#实验性稀疏-pbs)的密钥生成流程，
将 `None` 改为 `Some(cbs_config)`，再创建 `context.circuit_bootstrap_evaluator(&server)`。
CBS 与 CMUX 调用相同；两种 order 都支持经典和稀疏密钥。

输出为 accumulator 秘密下的 `NttGgsw`。密钥绑定输出布局及 trace/scheme-switch basis；
