...
- 本计划没有待执行子步骤；后续算法由用户按[候选清单](tfhe-next.md)选择，再建立独立计划。已完成步骤仅作恢复索引，不自动重启。
...
| B8 | NTRU 二元桶聚合 PBS | B8.1–B8.3 | NTT/Fourier 普通/ManyLUT 已验收；sparse CBS/MVB 未开放 |
...
- **不扩大算法支持**：GLWE CBS 的同层数输出 basis 复用与 NTRU CBS 的精确输出 basis 绑定分别保留；当时稀疏 CBS 保持拒绝，后续 NTT 接入见 B6.2。Boolean 加密器/解密器保持独立，保留 LWE 私钥/公钥加密。
...
- **结果**：[参数、噪声与成本](tfhe-sparse-cbs.md)。u64、n=728、N=1024、h=32、两种 order；BR `(10,4/5)` 配输出 `(8,3)` 通过逐行/层与非恒定 CMUX 余量检查。更小 gadget 尺度未满足本组逐层阈值，不因 CMUX 解码成功纳入通过范围。
- **验证**：同一 client 的经典/稀疏对照、桶内加密零/dummy 的精确噪声合成、独立整数卷积、公开经典输出逐元素参照、首次/复用零分配；默认/SIMD 统计一致。
- **边界**：复用同一 trace/scheme-switch key，完整调用成本与额外 BSK/workspace 已记录。临时原型已清理，正式接入见 B6.2；Fourier 及正式尾界不由本次原型关闭。
...
### B8.1：采样、映射与 NGSW 聚合原型（原型通过）

- **结果**：复用可逆前缀采样和 B4.1 的 `BucketMap`；u32/u64 NTT selector/dummy 系数聚合、单桶旋转与独立相位一致，初始化和单桶误差分别满足实际行噪声预算，首调用零分配。
- **资产与边界**：[NTRU 桶聚合专项](tfhe-ntru-sparse.md)记录两次条件化、小环反例及 `n=728,h=32,N=1024` 默认/SIMD 诊断。保留一个小型聚焦回归，临时诊断已清理；未新增高层 API、性能结论或完整 PBS 能力。固定客户端映射失败不重采秘密；完整链、安全和尾界留给各自验证，ternary 不在本组范围。

### B8.2：NTRU NTT 完整稀疏 PBS（已完成）

- **结果**：专用系数 NGSW key 接入原有 `ServerKey`/evaluator；普通/ManyLUT 共用批量量化、NLev 初始化、桶外积工作区和原 NTRU KS/提取。固定客户端后只重采 map，类型和错误归属见[专项](tfhe-ntru-sparse.md#5-正式-api-与接入边界)。
- **验收**：u32/u64 同客户端经典对照、独立旋转/相位、零 mask、奇偶桶数/空桶、错误先于写入/RNG 及首调用零分配通过；原单桶测试已消费正式 key。n=728 的普通/三输出、完整 server keygen、资源和误差见[成本](tfhe-ntru-sparse.md#7-b82-完整成本)。持久资产限于聚焦回归、一个基准和推荐示例。
- **边界**：实验性交付；sparse CBS（含独立材料入口）和 MVB 明确拒绝。条件秘密/公开映射的安全及完整失败率仍未认证，Fourier 留给 B8.3。

### B8.3：NTRU Fourier 原型与有条件接入（已完成）

- **结果**：显式奇数 fixed-weight binary，采样和导入生成都在 RNG 消费前拒绝偶数重量；保留 Fourier 稳定性筛选及固定客户端映射契约。系数 NGSW 桶聚合接入原有 server key/evaluator，普通/ManyLUT 支持两 FFT、u32/u64。
- **验收**：[独立相位、数值误差与完整成本](tfhe-ntru-sparse.md#8-b83-fourier-接入与独立验收)。小环回归保护初始化/行相位/聚合/整数外积参考；n=728、h=33 的所有桶、返回 KS、解码、零分配和默认/SIMD 通过。u32 在线快约 65%–71%；u64 RustFFT 小幅受益，TfheFFT 无稳定收益；密钥/生成成本增加，保持显式选择。
- **边界与资产**：sparse CBS（含独立材料入口）/MVB 明确拒绝；两次条件化的安全及完整失败率未认证。保留三个聚焦测试、一个完整成本基准和双 FFT 示例；大型诊断和打印已清理，记录与 CSV 留在专项文档。
