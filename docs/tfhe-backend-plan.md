...
B1–B7 已完成；B8.1–B8.2 的 NTRU NTT 桶聚合及完整 PBS 已完成，下一步 B8.3。B1.4–B1.7 整理四后端高层接口，分析与性能对照基线为 `66ae701`；B2–B8 保留原编号。当前任务与下一步记录在 [HANDOFF](../HANDOFF.md)，算法依据仍由覆盖分析和各专项文档维护。

## 执行方式

- 接下来推荐按 **`执行 B8.3` → 后续满足前置条件的步骤** 推进。`B1` 表示整个阶段；明确要求“执行 B1”时，完成其所有满足前置条件的剩余子步骤。
...
| B8 | NTRU 二元桶聚合 PBS | B8.1–B8.3 | B8.1–B8.2 NTT 完整链已验收；Fourier 待独立验证 |
