...
B1–B5 已完成，B6.1 的 NTT sparse CBS 原型已通过，下一步 B6.2。B1.4–B1.7 整理四后端高层接口，分析与性能对照基线为 `66ae701`；B2–B8 保留原编号。当前任务与下一步记录在 [HANDOFF](../HANDOFF.md)，算法依据仍由覆盖分析和各专项文档维护。

## 执行方式

- 接下来推荐按 **`执行 B6.2` → 后续满足前置条件的步骤** 推进。`B1` 表示整个阶段；明确要求“执行 B1”时，完成其所有满足前置条件的剩余子步骤。
...
| B6 | sparse CBS | B6.1–B6.3 | B6.1 NTT 原型已通过，待 B6.2 接入；Fourier 仍须 B6.3 独立验证 |
...
- **边界**：复用同一 trace/scheme-switch key，完整调用成本与额外 BSK/workspace 已记录。临时原型已清理，公开构造仍拒绝 sparse CBS；正式接入归 B6.2，Fourier 及正式尾界不由本次原型关闭。
