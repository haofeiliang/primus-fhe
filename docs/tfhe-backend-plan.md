...
B1–B6 与 B7.1–B7.2 已完成；GLWE 两后端 sparse CBS 已正式接入，NTRU ternary 已具备采样前置和 NTT 融合单步，下一步 B7.3。B1.4–B1.7 整理四后端高层接口，分析与性能对照基线为 `66ae701`；B2–B8 保留原编号。当前任务与下一步记录在 [HANDOFF](../HANDOFF.md)，算法依据仍由覆盖分析和各专项文档维护。

## 执行方式

- 接下来推荐按 **`执行 B7.3` → 后续满足前置条件的步骤** 推进。`B1` 表示整个阶段；明确要求“执行 B1”时，完成其所有满足前置条件的剩余子步骤。
...
| B7 | NTRU 经典 ternary | B7.1–B7.4 | B7.1 采样前置、B7.2 NTT 单步已完成；下一步 Fourier 单步，再接完整链 |
