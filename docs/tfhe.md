...
本文保存设计依据、职责边界和待决问题；实施任务见 [实施步骤](tfhe-plan.md)，算法方案见 [稀疏 PBS](tfhe-sparse-pbs.md) 与 [分解式 MVB](tfhe-mvb.md)。当前进度只记录在 [HANDOFF](../HANDOFF.md)，不在多份文件中维护状态。
...
| 首个 MVB 算法、适用模数和输出编码 | P4.1 已选固定尺度差分分解、GLWE NTT 奇数 q；证明、编码及实现边界见 [MVB 方案](tfhe-mvb.md) |
...
首版选择固定尺度差分分解，使用共同多项式 `V=(Delta*inv2)*sum(X^j)` 和小整数
因子 `W_i=(1-X)p_i`，精确恢复 `Delta*p_i`。GLWE NTT 先支持奇数 q、现有 Rounded
前半区输入和统一 Scaled 输出，保留经典/稀疏 BR、两种 order；量化步长为 1。
BK 在每个公开乘法之后 KS，KB 在共享 BR 之前 KS。

代数、Native 的偶尺度条件、真实差分范数、相关噪声、编码衔接、NTT 预处理绑定和
成本模型集中维护在 [MVB 专项文档](tfhe-mvb.md)。独立整数 oracle 已检查分解、所有
旋转、真实几何和解码充分条件；本阶段无 Rust API、加密执行或性能结论。P4.2 按该
方案实现具体类型及后端，P4.3 再作公平比较和应用入口。
