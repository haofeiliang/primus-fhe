...
**NTRU ternary：** 同类恒等式可以作用于 NGSW 控制，但当前 client key 还必须是可逆 NTRU 多项式。Native、二次幂 `N` 下至少要求 `f(1)` 为奇数；固定偶数非零重量 ternary 不会因重试而满足该条件。[B7.1–B7.2](tfhe-ntru-ternary.md)已完成底层采样/拒绝条件、padding、条件分布说明及 NTT NGSW 融合单步；Fourier 单步和完整链留给 B7.3/B7.4，TFHE 仍限制 binary。
