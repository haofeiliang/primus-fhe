...
P4.2 没有新增 benchmark；完整比较与应用入口见下节。阶段内不提供推测的生产参数，
也不把论文其他方案或所有候选后端变为本步的隐含交付。
...
该应用不需要加密 LUT、tree/LFBS、其他 FDFB 或摊销，此次不启动新研究步骤。
这些路线等待具体的函数保密、大域或批量吞吐需求。P4 的交付范围至此闭合。

本轮验证：GLWE NTT 默认/nightly SIMD 的 all-targets check、Clippy（`-D warnings`）、
各 15 项测试与示例运行通过；两配置各 27 项常驻基准完成计时和输出检查。共享 TFHE/
GLWE NTT 严格 rustdoc、格式、链接和 diff 检查通过。未改库源码/API 或增加测试，
未重跑其他后端、全 workspace、非 x86 或生产安全认证。
