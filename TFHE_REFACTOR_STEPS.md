...
两族公钥/私钥的三类 `_to` 已完成，后端 alias 直接继承。复用底层 LWE 内核，保留原分配返回路径；公开契约、双语 README 和四个 basic 示例已同步。

扩展现有客户端测试，覆盖 Native/Barrett、两种 GLWE order、奇偶明文模数及三种编码。固定随机输入下与分配接口的密文和 RNG 状态一致；消息/维数错误保持输出和 RNG 不变。复用已有分配计数器确认首次和后续 `_to` 调用均无分配。

- 格式、diff 检查与 workspace all-targets check 通过。
- 七包默认 / nightly SIMD 测试各 40 项通过；两配置 all-targets Clippy（`-D warnings`）通过。
- 七包严格 rustdoc 和四个 basic 示例独立运行通过。

未运行性能计时或完整 workspace 测试。S4 无阻塞项，下一步为 S5。
