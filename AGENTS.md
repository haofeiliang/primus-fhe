...
- 开始时检查 `git status`、相关暂存/未暂存差异及 [HANDOFF](HANDOFF.md)，保留用户修改和暂存状态。只按目标读取其链接，不自动重开已完成的全库整理。
...
- 成员见根 `Cargo.toml` 的 `crates/*` 和 `xtask`；跨层修改从底层向调用方推进。`temp/` 仅供参考，不修改其中外部项目。仓库已配置 `target-cpu=native`，不在命令中重复设置。
...
- Rust 代码审查、复审或基于源码的重构分析使用 [primus-review](.agents/skills/primus-review/SKILL.md)，由其确定深度、覆盖和委派。常规实现验证、仅依据已有报告规划，不自动升级为完整复审。
- **AGENTS** 保存开发规范；**HANDOFF** 只保存当前任务、有效边界、未决项和下一步，替换旧状态，不追加日志或复制算法全文。
