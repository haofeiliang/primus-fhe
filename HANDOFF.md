...
- 已完成：**P1.1、P1.M、P1.2、P1.3、P1.R、P1.4、P2.1、P2.2、P2.3、P3.1、P3.2、P3.3**。设计和测量依据见 [TFHE 总览](docs/tfhe.md)，完成条件见 [实施步骤](docs/tfhe-plan.md)。
- 进行中：无；本步剩余：无。下一步：**P3.4 聚合、表示与工作区优化**，先读 [P3 实现契约](docs/tfhe-sparse-pbs.md)及其 P3.3 实现入口；尚未开始 P3.4。
...
- 测试/基准有效边界：P3.2 保留两个私有匹配测试、两个稀疏 key 集成测试；P3.3 只新增一项表驱动 BR 集成测试，覆盖小环全指数、公开空桶、奇偶桶数、k=2、截断 basis、真实加密输入、输出写入前拒绝和零分配，未增加 benchmark。ManyLUT 使用代表性消息与 `k=1/3/4`，三路 CBS 使用三层和 `1→0`；两种 GLWE order、两种 FFT 和错误边界保留。P2.3 增加两个共享整数 oracle/拒绝测试，将既有四后端 fixture 改为 `15→8` 并遍历全域，共增加 135 次小参数 PBS，不增加 keygen 或测试程序。P2.2 的两个打包输入继续复用同一 fixture。Criterion 仍为 8 个 target、97 项；GitHub CI 未执行 benches。
- 当前验证：P3.3 及批量量化接口：共享 TFHE 的 11 项、NTT 后端的 13 项测试在默认及 nightly SIMD 均通过；两包默认 all-targets check/Clippy、严格私有 rustdoc、格式和 diff 检查通过。复用既有整数 oracle 覆盖标量/切片量化和步长，测试数不增加。稀疏与经典路径相位/解码对照及有限样本误差见专项文档；步长 1/相等模数的批量量化快速路径经临时 Criterion 对照出现退化，未保留，见[取舍记录](docs/tfhe-sparse-pbs.md#批量量化快速路径的取舍)。BR 性能、完整稀疏 PBS、全 workspace 或生产安全/尾概率未验证。P3.2 已有匹配性能选择见 [匹配表示与测量](docs/tfhe-sparse-pbs.md#p32-匹配表示与测量)。
