...
- **B8.1 NTRU NTT 桶聚合单步原型通过，下一步 B8.2，不自动启动**：[采样、聚合与预算](docs/tfhe-ntru-sparse.md)。复用 `generate_padded_pair` 和共享 `BucketMap`，fixed-weight binary 客户端保持同一有效前缀及零 padding；映射失败只重采 map。u32/u64 的独立行相位、selector/dummy 聚合、NLev 初始化和单桶误差预算及首调用零分配通过。n=728、h=32、N=1024 的默认/SIMD 三类单桶诊断一致；只保留小回归，临时大参数代码已清理；`just tfhe`（严格 rustdoc）和 `just tfhe-simd` 通过。未新增 sparse server key 或完整 PBS API，也无性能结论。B8.2 需独立验证串联桶、量化、返回 KS 和资源；首轮不开放 sparse CBS/MVB。可逆性与公开映射的联合条件分布已写明，安全估计和完整失败率未认证。
