...
- 已完成：**P1.1、P1.M、P1.2、P1.3、P1.R、P1.4、P2.1、P2.2**。设计和测量依据见 [TFHE 总览](docs/tfhe.md)，完成条件见 [实施步骤](docs/tfhe-plan.md)。
- 进行中：无；本步剩余：无。下一步：**P2.3 奇数明文模数全域**，入口为实施步骤对应章节及共享 LUT 几何；尚未开始 P2.3。
...
- 测试/基准有效边界：保留 43 项独立测试。ManyLUT 用代表性消息与 `k=1/3/4`，三路 CBS 用三层和 `1→0`；两种 GLWE order、两种 FFT 和错误边界保留。P2.2 新增两个共享小域 oracle/拒绝测试，在既有四后端 fixture 中各配置增加两个双输入 PBS，共增加 18 次 PBS，不增加 keygen 或测试程序。Criterion 仍为 8 个 target、97 项；GitHub CI 未执行 benches。
- 当前验证：P2.2 的 `just tfhe`、`just tfhe-simd`、workspace all-targets check 通过；默认/nightly SIMD 各 43 项 TFHE 测试，相关 Clippy 与默认文档通过。修改后的 NTRU NTT basic 示例在默认/SIMD 下均运行通过。小域 oracle 核对真实编码的舍入偏差和线性误差，四后端覆盖打包/PBS 零在线分配。未重做性能计时、其余 workspace 测试及非 x86/生产噪声安全验证；历史验证与测量留在总览。
