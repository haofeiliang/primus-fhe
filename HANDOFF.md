...
- 已完成：**P1.1、P1.M、P1.2、P1.3、P1.R、P1.4、P2.1、P2.2、P2.3、P3.1、P3.2、P3.3、P3.4、P3.5、P4.0、P4.1、P4.2、P4.3**。设计和测量依据见 [TFHE 总览](docs/tfhe.md)，完成条件见 [实施步骤](docs/tfhe-plan.md)。
- 进行中：无；本步剩余：无。**P1–P4 已完成**，下一步由用户确定；ternary 是已表达的后续方向，automorphism 暂缓，均不自动开启。
- P4.3 有效边界：相同 Scaled 阈值输出、n=728、N=1024、两种 order 与经典/稀疏密钥，默认/SIMD 比较完整 PBS。三输出 MVB 相对独立 PBS 约快 2.86–3.03 倍，17 输出约快 15.28–17.36 倍；三输出相对交错没有统一优势，17 输出交错容量不足。程序/构造成本、额外 8 KiB evaluator 缓冲、真实范数与有限相位/量化误差另行测量。BK 共享 KS 原型在大输出组有收益，但小组无稳定收益且放大 KS 噪声，保留逐输出 KS；原型已删除。新增阈值示例，无新策略 API/常驻测试。具体测量、选择条件与后续触发只维护在 [P4.3 记录](docs/tfhe-mvb.md#8-p43-测量与应用选择)，不继续扩展其他 MVB 研究路线。
- P4.2 有效边界：GLWE NTT 奇数 q 的固定尺度差分分解 MVB 已实现。共享 `FactorizedLookupTable::try_new(D, N, k, input_codec, output_codec, function)` 复用 Rounded 前半区/短前缀几何，统一 Scaled 输出、任意正输出数、步长 1；callback 外层遍历输出。`NttFactorizedLookupTable::new` 消费系数产物、原地变换因子并借用 context；执行时检查 context 实例相同。context 提供 `compile_factorized_lookup_table_fn` 和 `factorized_evaluator`；独立 evaluator 的 `_to` 复用私有阶段，经典/稀疏均适用，BK 逐输出乘后 KS，KB 前置 KS 一次，仅额外一个完整 NTT GLWE 缓冲、在线零分配，普通 evaluator 不变。Native/Fourier/NTRU、odd full-domain、CBS、ternary/unfolding 不纳入首版；代数、噪声/成本、接口与验证只维护在 [MVB 专项文档](docs/tfhe-mvb.md)。实际性能与应用示例见同文档 P4.3。
...
- LUT 源码边界：四个公开类型统一从 `lookup_table` 导出，`single.rs` 集中 `LookupTable` 的全部 API，`interleaved.rs` 管理输出布局，`bivariate.rs` 绑定输入打包，`factorized.rs` 保存共同多项式与差分因子；`compile/` 分别实现前半区与奇数全域。编码/域/槽容量在编译入口检查，中点与负循环尾部填充共用。奇数全域仍是单输出构造方式；隐藏 helper 改名为 `front_half_domain_len`，family/CBS 调用方已同步。命名区分 `input_ciphertext_modulus` 与 `coefficient_modulus`、布局 `padded_output_count`（补齐输出数）/`coefficients_per_output`（每输出系数数）与执行端 `rotation_step`；CBS 使用 `lookup_table_padded_output_count()`，旋转量化统一使用 `rotation::RotationQuantizer`。见 [源码组织](crates/primus_tfhe/README.zh_CN.md#源码组织)。
...
- 有效未决项：P3 的实验方案已收敛，但固定重量/补零目标/evaluation keys 的生产安全、成功映射条件分布的影响和完整 PBS 尾界仍未认证；不得用功能测试关闭。首个 MVB 已完成功能、性能及应用比较；代数和有限功能样本不能推出生产失败率。居中/shifted 的正式接入和带辅助密钥的漂移抑制为后续候选，不构成 P1–P4 的隐含交付。具体内容只维护在对应文档中。
- 测试/基准有效边界：P3.2 保留两个私有匹配测试、两个稀疏 key 集成测试；P3.3 只新增一项表驱动 BR 集成测试，覆盖小环全指数、公开空桶、奇偶桶数、k=2、截断 basis、真实加密输入、输出写入前拒绝和零分配；P3.4 将加密场景改为 k=2 以覆盖跨块及较短尾块，不增加测试数。ManyLUT 使用代表性消息与 `k=1/3/4`，三路 CBS 使用三层和 `1→0`；两种 GLWE order、两种 FFT 和错误边界保留。P2.3 增加两个共享整数 oracle/拒绝测试，将既有四后端 fixture 改为 `15→8` 并遍历全域，共增加 135 次小参数 PBS，不增加 keygen 或测试程序。P2.2 的两个打包输入继续复用同一 fixture。P3.5 新增一项完整 sparse PBS 测试，CBS 拒绝复用现有 fixture；成本组 8 项完整 PBS、2 项 keygen 替换原始 BR 四项基准，小组/内存/相位与匹配诊断不进入常驻测试或基准；GitHub CI 未执行 benches。 P4.2 仅新增两个共享 MVB 测试和一个 GLWE NTT 集成测试，复用经典/稀疏与两种 order fixture 检查 1/3/17 输出、同尺度独立/交错对照和拒绝边界。P4.3 新增一个 MVB target，20 项在线比较与 7 项构造/准备，TFHE Criterion 合计 10 个 target、134 项；没有增加常驻测试，临时 KS 原型和统计程序已删除。
- 当前验证：P4.3 GLWE NTT 默认/nightly SIMD 的 all-targets check、Clippy（-D warnings）和测试均通过（各 15 项），阈值示例两配置运行通过。常驻 MVB 两配置各 27 项完成实际计时与输出验证，临时 KS 对照两配置各两轮完成；默认内存/误差诊断覆盖正式路径和共享 KS，在线零分配。共享 TFHE/GLWE NTT 严格 rustdoc、格式、文档链接和 diff 检查通过。本轮只新增基准/示例/文档，未修改库源码或 API；未重跑其余后端/全 workspace 测试、非 x86 或生产安全/失败概率认证。
