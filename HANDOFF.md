...
- **B7.1–B7.2 NTRU ternary 采样与 NTT 单步已完成，下一步 B7.3，不自动启动**：[秘密采样与融合单步](docs/tfhe-ntru-ternary.md)。底层 `generate_padded_pair` 保持零 padding，Native 固定偶数非零重量在采样前拒绝，输出为后端接受条件下的条件分布。`NttNgsw::cmux_ternary_monomial_to` 组合正负控制后执行一次外积；工作区复用 digit 缓冲保存单项式因子，零指数精确复制。独立系数相位、两次 CMUX 和零分配通过；u32/u64 默认/SIMD 单步快 35%–42%，scratch 多 8/40 KiB。TFHE 参数/导入密钥仍限制 binary，完整链、整把 BSK 的性能及误差尾界留给 B7.4。lattice/NTRU 默认与 SIMD 检查、测试、Clippy、两条 TFHE recipe 和严格 rustdoc 通过；B7.3 下一步验证 Fourier NGSW 单步。
