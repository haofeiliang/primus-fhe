...
- 已完成：**P1.1、P1.M、P1.2、P1.3、P1.R、P1.4、P2.1、P2.2、P2.3、P3.1、P3.2、P3.3、P3.4、P3.5**。设计和测量依据见 [TFHE 总览](docs/tfhe.md)，完成条件见 [实施步骤](docs/tfhe-plan.md)。
- 进行中：无；本步剩余：无。下一步：**P4.1 MVB 算法与编码选型**，先读实施步骤的 P4 约束与 [TFHE 总览](docs/tfhe.md)。P3 完整 PBS 已接入，稀疏 CBS 保持不支持。
...
- 当前验证：P3.5 的 NTT 后端 14 项测试在默认及 nightly SIMD 均通过；两配置 all-targets check/Clippy、严格私有 rustdoc、workspace all-targets check、格式和 diff 检查通过。两组 P3 参数完成两种 order、普通/交错的同秘密经典/稀疏完整 PBS 对照，以及 keygen、实际分配、相位余量和匹配诊断；默认/SIMD 功能与内存记录一致，见专项文档。未跑全 workspace 数值测试、非 x86 路径或稀疏 CBS；生产安全与完整尾概率仍未认证。
