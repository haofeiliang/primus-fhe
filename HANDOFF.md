...
- 已完成：**P1.1、P1.M、P1.2、P1.3 布局 API 与调用方迁移（含 LUT 模块拆分）**。设计和测量依据见 [TFHE 总览](docs/tfhe.md)，完成条件见 [实施步骤](docs/tfhe-plan.md)。
- 下一步：**P1.4 阶段验收与资产整理**，按最终迁移范围汇总同配置构造/在线 PBS 性能，并核对验证矩阵与维护资产；尚未开始 P2。
...
- 有效未决项：P1.4 最终性能验收；奇数全域条件在 P2.3 收敛；PBC 参数、安全/噪声条件在 P3.1 收敛；首个 MVB 算法与缩放在 P4.1 收敛。具体内容只维护在对应文档中。
- 本步验证：七个 TFHE crate 默认与 nightly SIMD 的 all-targets check / Clippy、各 39 项测试通过，覆盖 `k=3,s=4`、补零/回调边界、输出数量拒绝、四后端 PBS 和三路 CBS；构造仍一次分配，在线 `_to` 仍零分配。Workspace check / Clippy、格式、七包文档及共享库严格 rustdoc（含私有模块）通过。模块拆分保持公开 API、计算逻辑和内联标记；未重新计时构造/在线 PBS，未重跑示例、benchmark、非 x86 或无关底层包测试；P1.4 进行最终同配置比较。
