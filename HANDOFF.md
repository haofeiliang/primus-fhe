...
- **B7.1 NTRU ternary 采样前置已完成，下一步 B7.2，不自动启动**：[秘密采样专项](docs/tfhe-ntru-ternary.md)。底层 NTT/Fourier 改用 `generate_padded_pair`，完整长度生成复用该路径；五种 ternary 候选分布按 active prefix 采样并保持零尾部。Native 固定偶数非零重量在采样前报 `NonInvertibleSecretKey`，NTT 逐候选求逆，Fourier 另检查数值逆元；统一最多 1024 次。实际输出是后端接受条件下的条件分布，安全估计与完整误差尾界未证明。TFHE 参数/导入密钥及 binary CMUX 限制保留；底层默认/SIMD check/test/Clippy、两条 TFHE recipe 和 NTRU 严格 rustdoc 通过。B7.2 下一步验证 NTT NGSW ternary 单步。
